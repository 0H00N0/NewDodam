...
public class CommunityController {
    ...
    @GetMapping
    public CommunityDTO.PageResp<CommunityDTO.Resp> list(
            @RequestParam(name = "bcanum", required = false) Long bcanum,
        ...
        return CommunityDTO.PageResp.<CommunityDTO.Resp>builder()
    ...
    public ResponseEntity<Long> create(@RequestBody CommunityDTO.CreateReq req,
        ...
        if (req.getBcanum() == null) req.setBcanum(3L);
    ...
    @PutMapping(value = "/{bnum}", consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<Void> update(@PathVariable("bnum") Long bnum,
                                       @RequestBody CommunityDTO.UpdateReq req,
                                       Authentication authentication) {
        if (authentication == null) throw new ResponseStatusException(HttpStatus.UNAUTHORIZED);
