...
public class PlanPaymentMethodController {
    ...
    public ResponseEntity<?> register(@Valid @RequestBody PlanPaymentRegisterReq req, HttpSession session) {
        ...
        PlanCardMeta meta = new PlanCardMeta(null, null, null, null, null, false, req.getCustomerId());
        ...
        // 선택 방어: rawJson이 이번 billingKey와 정확히 일치할 때만 보강
        try {
            if (org.springframework.util.StringUtils.hasText(req.getRawJson())) {
                JsonNode raw = mapper.readTree(req.getRawJson());
    ...
    private void safeMergeMeta(PlanPaymentEntity e, PlanCardMeta meta) {
        ...
        if (!StringUtils.hasText(e.getPayLast4()) && StringUtils.hasText(meta.getLast4())) e.setPayLast4(sanitizeLast4(meta.getLast4()));
    ...
    private void safeMergeMetaMap(PlanCardMeta meta, Map<String, String> map) {
        ...
        if (!StringUtils.hasText(meta.getLast4()) && StringUtils.hasText(map.get("last4"))) meta.setLast4(sanitizeLast4(map.get("last4")));
    ...
    private static String sessionKeyFor(String billingKey) { return "bkmeta:" + billingKey; }
    ...
    private void tryUpdateCardMeta(Long payId, String billingKey) {
        try {
            ...
            // payId로 검사: 해당 결제수단의 payKey가 billingKey와 일치해야만 업데이트
            paymentRepo.findById(payId).ifPresent(p -> {
                if (!billingKey.equals(p.getPayKey())) {
                    log.warn("[REGISTER] skip meta update: payId={} has different payKey({})", payId, p.getPayKey());
                    return;
                }
                boolean changed = false;
                String last4 = sanitizeLast4(cardMeta.getLast4());
                if (StringUtils.hasText(cardMeta.getBin())   && !cardMeta.getBin().equals(p.getPayBin()))     { p.setPayBin(cardMeta.getBin());       changed = true; }
                if (StringUtils.hasText(cardMeta.getBrand()) && !cardMeta.getBrand().equals(p.getPayBrand())) { p.setPayBrand(cardMeta.getBrand());   changed = true; }
                if (StringUtils.hasText(last4)               && !last4.equals(p.getPayLast4()))               { p.setPayLast4(last4);                 changed = true; }
                if (StringUtils.hasText(cardMeta.getPg())    && !cardMeta.getPg().equals(p.getPayPg()))       { p.setPayPg(cardMeta.getPg());         changed = true; }
                if (changed) paymentRepo.save(p);
                if (changed) {
                    log.info("[REGISTER] cardMeta updated for payId={} bin={} brand={} last4={} pg={}",
                            payId, p.getPayBin(), p.getPayBrand(), p.getPayLast4(), p.getPayPg());
                }
            });
        } catch (Exception e) {
    ...
    private void saveIssuedCardMeta(String rawJson, String fallbackPayIdOrNull) {
        ...
        // 1) billingKey 기준으로 업데이트
        if (StringUtils.hasText(meta.getBillingKey())) {
            boolean updated = persistCardMetaByKey(meta.getBillingKey(), meta.getBin(), meta.getBrand(), meta.getLast4(), meta.getPg());
            log.info("[PMCtrl] update by key updated={}, key={}", updated, meta.getBillingKey());
            if (updated) return;
        }

        // 2) fallback: payId로 시도
        if (StringUtils.hasText(fallbackPayIdOrNull)) {
            try {
                Long payId = Long.valueOf(fallbackPayIdOrNull);
                boolean updated = persistCardMetaById(payId, meta.getBin(), meta.getBrand(), meta.getLast4(), meta.getPg());
                log.info("[PMCtrl] update by id updated={}, payId={}", updated, payId);
            } catch (Exception ignore) {}
        }
    }

    /* ===== 내부 저장 헬퍼 ===== */
    private boolean persistCardMetaById(Long payId, String bin, String brand, String last4, String pg) {
        if (payId == null) return false;
        return paymentRepo.findById(payId).map(p -> {
            boolean changed = false;
